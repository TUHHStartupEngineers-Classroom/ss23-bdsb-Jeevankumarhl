...
# Createing the line plot with the cumulative cases over time
```{r}
...
## Visualising distribution of the mortality rate (deaths / population)

# Reading data and adjust country names
```{r}
...
# Grouping the adjusted data by location and keep latest entry for each location
```{r}
...
# Joining the latitude/longitude data with COVID-19 data
```{r}

plot_data <- left_join(world, adjusted_data_latest, by = c("region" = "location"))
```

# Visualising distribution of the mortality rate or case-fatality rate
```{r}
